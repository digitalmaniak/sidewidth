...
begin
  ...
  where
    ...
    -- Apply filters based on SideWidth (vote_stddev) for specific sorts
    and (
        case 
            when sort_by = 'divided' then p.vote_stddev > 50 and p.vote_count >= 2
            when sort_by = 'consensus' then p.vote_stddev < 50 and p.vote_count >= 2
            else true 
        end
    )
  order by
